  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-[#1a1a1a] p-8 rounded-xl shadow-2xl w-full max-w-4xl relative overflow-hidden transition-colors">
        <button onClick={onClose} className="absolute top-8 right-8 p-2 text-gray-400 hover:text-orange-600 transition-colors rounded-xl dark:hover:bg-[#151515]"><FaTimes size={18} /></button>

        <header className="mb-8">
          <div className="flex items-center gap-3 text-orange-600 mb-2">
            <FaPeopleGroup className="text-2xl" />
            <h3 className="text-xs font-semibold uppercase tracking-[3px]">Collaboration Unit</h3>
          </div>
          <h2 className="text-xl font-bold text-gray-800 dark:text-white tracking-tighter">Team Personnel</h2>
        </header>
        <div className="h-[1px] bg-gray-100 dark:bg-[#1a1a1a] w-full mb-6" />

        <div className="grid gap-6 overflow-y-auto max-h-[60vh] pr-4 no-scrollbar sm:grid-cols-2 lg:grid-cols-3">
          {teamDetails.length > 0 ? (
            teamDetails.map((member) => (
              <div key={member._id} className="flex items-center p-5 bg-gray-50/50 dark:bg-[#151515]/30 border border-gray-100 dark:border-[#1a1a1a] rounded-[2rem] hover:border-orange-500/20 transition-all group">
                <img src={`/avatars/${member.avatar || 1}.jpg`} alt={member.name} className="w-12 h-12 rounded-2xl border-2 border-white dark:border-[#0a0a0a] shadow-lg group-hover:scale-110 transition-transform" />
