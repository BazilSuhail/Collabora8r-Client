...
const MyTask = ({ task, user, creator }) => {
  ...
  return (
    <motion.div
      ...
      className="p-6 bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-[#1a1a1a] rounded-2xl shadow-sm hover:shadow-xl hover:border-orange-500/20 dark:hover:border-orange-500/10 cursor-pointer transition-all duration-300 group mb-6"
    >
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-6'>
        <div className="flex items-center gap-5">
          ...
          <div>
            <h3 className='font-bold text-lg text-gray-800 dark:text-gray-100 mb-2 truncate max-w-xs md:max-w-md'>
              {task.title}
            </h3>
        ...
        <div className="flex flex-col items-end gap-2 text-right">
          <div className='flex items-center gap-2 px-3 py-1.5 bg-red-50 dark:bg-red-900/10 rounded-lg text-red-600 dark:text-red-400'>
            ...
            <span className='text-sm font-bold'>{new Date(task.dueDate.$date || task.dueDate).toLocaleDateString()}</span>
          </div>
      ...
      <div className='mt-6 pt-6 border-t border-gray-50 dark:border-[#1a1a1a] flex items-center justify-between'>
        ...
        <button className="text-orange-600 text-[10px] font-black uppercase tracking-[3px] opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 transition-all duration-500">
          Report &rarr;
        </button>
      </div>
    </motion.div>
    ...
    <div className="space-y-6">
      {tasks.length === 0 ? (
        <div className='flex flex-col items-center justify-center py-20 opacity-60 grayscale scale-75'>
          ...
          <p className="text-center font-bold text-gray-400 mt-4 tracking-widest uppercase text-xs">Zero directives currently active in sector</p>
        </div>
      ) : (
        <>
