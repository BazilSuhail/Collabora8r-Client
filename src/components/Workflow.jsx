...
const Workflow = () => {
  ...
  return (
    <DndProvider backend={backend}>
      <div className="min-h-screen p-6 bg-[#fcfaf8] dark:bg-[#000000] transition-colors duration-300">
        <div className="flex sm:flex-row flex-col sm:justify-between sm:items-center">
          <div className="flex items-center space-x-2">
            <FaPeopleRoof className="text-2xl text-gray-600 dark:text-gray-400" />
            <h2 className="text-2xl md:text-3xl text-gray-800 dark:text-white font-bold tracking-tight">Workflow Manager</h2>
          </div>
        </div>
        <p className='mt-1 text-sm text-gray-500 dark:text-gray-400 lg:ml-8 mb-6'>
          Strategize and manage your task statuses across all active projects.
        </p>
        <div className='w-full h-[1px] bg-gray-200 dark:bg-[#1a1a1a] mb-10'></div>

        {Object.values(tasks).every((taskArray) => taskArray.length === 0) ?
          <div className="flex flex-col items-center justify-center py-20">
            <div className='relative mb-8'>
              <div className="absolute inset-0 bg-orange-500/20 blur-3xl rounded-full" />
              <img src="/Resources/4.png" alt='No tasks' className='relative w-64 md:w-80 opacity-80 dark:opacity-40 grayscale group-hover:grayscale-0 transition-all duration-700' />
            </div>
            <p className="text-xl font-bold text-gray-800 dark:text-white mb-2">Workspace clear</p>
            <p className="text-gray-500 dark:text-gray-400 text-center max-w-xs">You don't have any assigned tasks at the moment. Time to start something new?</p>
          </div>
          :
          <div className="max-w-[1600px] mx-auto">
            ...
            <motion.button
              ...
              className={`mt-10 px-8 py-3 rounded-2xl text-sm font-bold flex items-center gap-3 transition-all duration-300 shadow-xl
                ${Object.keys(updatedTasks).length === 0 
                  ? 'bg-gray-200 dark:bg-[#151515] text-gray-400 cursor-not-allowed shadow-none' 
                  : 'bg-orange-600 text-white hover:bg-orange-700 hover:-translate-y-1 shadow-orange-600/20'}`} >
