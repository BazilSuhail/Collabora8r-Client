...
const JoinedTaskDetails = () => {
  ...
  return (
    <main className="min-h-screen bg-[#fcfaf8] dark:bg-[#000000] p-6 transition-colors duration-300">
      <div className="max-w-[1600px] mx-auto grid xl:grid-cols-7 grid-cols-1 gap-8 pt-4">
        
        <section className='lg:col-span-5 flex flex-col gap-6'>
          {/* Directive Header */}
          <div className='bg-white dark:bg-[#0a0a0a] p-8 border border-gray-100 dark:border-[#1a1a1a] rounded-3xl shadow-sm'>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
              <div className="flex items-center gap-5">
                <div className='w-16 h-16 bg-orange-100 dark:bg-orange-500/10 rounded-2xl flex items-center justify-center text-3xl text-orange-600'>
                  <FaClipboardList />
                </div>
                <div>
                  <h1 className="text-2xl md:text-3xl font-black text-gray-800 dark:text-white tracking-tighter">
                    {task.title}
                  </h1>
                  <div className='flex items-center gap-2 mt-1'>
                    <span className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Target Sector Lead:</span>
                    <span className="text-xs font-black text-orange-600 dark:text-orange-500 uppercase underline decoration-orange-500/20 underline-offset-4">{creatorName}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6 border-t border-gray-50 dark:border-[#1a1a1a]">
              <div className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-[#151515] rounded-2xl border border-transparent">
                ...
                <div>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Operational Deadline</p>
                  <p className="font-black text-gray-800 dark:text-white">{new Date(task.dueDate.$date || task.dueDate).toLocaleDateString()}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-[#151515] rounded-2xl border border-transparent">
                <div className="w-1.5 h-full bg-green-500 rounded-full" />
                <div>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Current Authorization</p>
                  <p className="font-black text-gray-800 dark:text-white uppercase">{task.status}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className='bg-white dark:bg-[#0a0a0a] p-8 border border-gray-100 dark:border-[#1a1a1a] rounded-3xl shadow-sm'>
            <div className='flex items-center gap-3 text-orange-600 mb-6'>
              ...
              <h3 className='text-sm font-black uppercase tracking-widest'>Mission Directives</h3>
            </div>
            <p className="text-gray-600 dark:text-gray-400 leading-relaxed pl-8 border-l-2 border-orange-500/10 italic">
              {task.description}
            </p>
          </div>

          {/* Operational Logs (Comments) */}
          <div className='bg-white dark:bg-[#0a0a0a] p-8 border border-gray-100 dark:border-[#1a1a1a] rounded-3xl shadow-sm flex-1'>
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3 text-orange-600">
                <BsPeopleFill className="text-2xl" />
                <h3 className="text-sm font-black uppercase tracking-widest">Operational Intel</h3>
              ...
              <AnimatePresence>
                {comments.length === 0 ? (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-12 text-center opacity-40">
                    ...
                    <p className="text-sm font-bold tracking-widest uppercase">Intel void detected.</p>
                  </motion.div>
                ) : (
                  comments.map((comment) => (
                    <motion.div 
                      ...
                      className="group relative pl-12 pb-8 border-l-2 border-gray-50 dark:border-[#1a1a1a] last:border-transparent"
                    >
                      <img
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          ...
                          <span className="text-[10px] font-bold text-gray-400 tracking-tighter uppercase">{comment.user?.email || 'OFFLINE'}</span>
                        </div>
                        ...
                        <div className='mt-4 bg-gray-50 dark:bg-[#151515] p-4 rounded-2xl border border-orange-500/20'>
                          ...
                          <div className="flex justify-end gap-2 mt-4">
                            <button
                              ...
                              className="px-4 py-2 text-xs font-black uppercase text-gray-500 hover:text-gray-800"
                            >
                              Abort
                            </button>
                            <button
        ...
        <section className='lg:col-span-2 flex flex-col gap-6'>
          {/* Progress Visualizer */}
          <div className="p-8 bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-[#1a1a1a] rounded-3xl shadow-sm text-center">
            <div className="flex items-center gap-3 text-orange-600 mb-8 border-b border-gray-50 dark:border-[#1a1a1a] pb-4">
              ...
              <h3 className="text-sm font-black uppercase tracking-widest">Synchronization</h3>
            </div>
            ...
            <div className="relative inline-flex items-center justify-center mb-6">
              ...
              <div className="absolute flex flex-col items-center">
                <span className="text-3xl font-black text-gray-800 dark:text-white">{clampedProgress}%</span>
              </div>
            </div>

            <div className={`py-2 px-4 rounded-xl text-[10px] font-black uppercase tracking-[2.5px] mb-6 shadow-inner ${
            ...
            {task.assignedTo === user._id && (
              <button 
                onClick={toggleModal} 
                className="w-full bg-white dark:bg-[#0a0a0a] text-orange-600 border-2 border-orange-600 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-orange-600 hover:text-white transition-all shadow-sm active:scale-95"
              >
                Modify Authorization
              </button>
            )}
            <p className='text-[9px] text-gray-400 dark:text-gray-500 mt-4 font-bold uppercase tracking-tighter'>
              Synchronization lock initiates after deadline epoch.
            </p>
          </div>

          {/* Comment Transceiver */}
          <div className="p-8 bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-[#1a1a1a] rounded-3xl shadow-sm">
            <div className="flex items-center gap-3 text-orange-600 mb-6">
              <IoPersonSharp className="text-xl" />
              <h3 className="text-xs font-black uppercase tracking-widest">Intel Transceiver</h3>
            </div>
            <div className="relative">
              <textarea
                ...
                className="w-full p-6 bg-gray-50/50 dark:bg-[#151515] dark:text-white border-2 border-transparent focus:border-orange-500/20 rounded-2xl outline-none transition-all h-40 resize-none text-sm font-medium"
              />
              <button 
            ...
            <p className="text-[9px] font-bold text-gray-400 mt-3 text-center uppercase tracking-tighter">Transmission sent to {creatorName}</p>
          </div>
        </section>
      </div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md z-[1000]">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-[#1a1a1a] rounded-3xl shadow-2xl w-full max-w-md p-8 relative"
            >
              ...
              <header className="mb-8">
                <h3 className="text-2xl font-black dark:text-white tracking-tighter">Modify Sector Directive</h3>
                <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mt-1">Re-evaluating operational metrics.</p>
              </header>
              ...
              <div className="space-y-8">
                ...
                <button 
                  ...
                  className="w-full bg-orange-600 hover:bg-orange-700 disabled:bg-orange-600/50 py-4 rounded-2xl text-white font-black text-xs uppercase tracking-[3px] transition-all shadow-lg shadow-orange-600/20 active:scale-95"
                >
