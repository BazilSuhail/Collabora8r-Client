...
const JoinedProjectDetails = () => {
  ...
  const [filteredTasks, setFilteredTasks] = useState([]); // To filter tasks
  ...
  const [view, setView] = useState('all'); // Toggle between "all" and "my"
  ...
  useEffect(() => {
    ...
    const fetchProjectTasks = async () => {
      try {
        ...
        setFilteredTasks(tasks); // By default, show all tasks
  ...
  return (
    ...
    {project ? (
      <>
        ...
        <TeamMembers teamDetails={teamDetails} />

         */}

        {/* Tasks section with toggle view */}
        <div className="bg-white shadow-lg rounded-lg p-5">
          ...
          <div className="mb-4">
            <button
              onClick={() => setView('my')}
              className={`mr-4 px-4 py-2 ${view === 'my' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
            >
              My Tasks
            </button>
            <button
