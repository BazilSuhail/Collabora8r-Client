...
const AdminProjectList = () => {
  ...
  const { adminProjects, fetchAdminProjects } = useProjectStore()
  ...
  return (
    <div className='min-h-screen bg-[#fcfaf8] dark:bg-[#000000] flex flex-col p-6 transition-colors duration-300'>
      ...
      <p className='mt-1 text-sm text-gray-500 dark:text-gray-400 lg:ml-8 mb-6'>Manage and administrate your established project ecosystems.</p>
      <div className='h-[1px] bg-gray-200 dark:bg-[#1a1a1a] w-full mb-8'></div>

      {error &&
        <div className='p-4 bg-red-100 dark:bg-red-900/10 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/20 rounded-xl mb-6'>{error}</div>
      }
      ...
      {adminProjects.length <= 0 ? (
        <div className='flex flex-col items-center justify-center py-20'>
          <div className="relative mb-8">
            ...
            <img src="/Resources/3.png" alt='No projects' className='relative w-64 md:w-80 opacity-80 dark:opacity-40 grayscale' />
          </div>
          <p className="text-lg font-bold text-gray-800 dark:text-white mb-4">You haven't created any Project</p>
          <button onClick={() => setShowModal(true)} className="px-8 py-3 bg-orange-600 text-white rounded-2xl font-bold shadow-lg shadow-orange-600/20 hover:bg-orange-700 transition-all">
            Create One Now
          </button>
        </div>
      ) : (
        <div className="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          {adminProjects.map((project) => (
            <motion.div key={project._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="group h-[320px] bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-[#1a1a1a] rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col">
              <div className='p-4 bg-gray-50 dark:bg-[#151515] border-b border-gray-100 dark:border-[#1a1a1a] flex items-center justify-between'>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white shadow-lg ${projectColors[project._id] || 'bg-gray-400'}`}>
                ...
                <span className='px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400'>Active</span>
              </div>
              ...
              <div className='p-5 flex-1 space-y-3'>
                <div className='flex items-center gap-3 text-sm'>
                  ...
                  <span className='font-bold text-gray-800 dark:text-white'>{project.team?.length || 0}</span>
                </div>
                <div className='flex items-center gap-3 text-sm'>
                  <GrTask className='text-orange-500 text-lg' />
                  <span className='text-gray-500 dark:text-gray-400 font-medium'>Work Units:</span>
                  <span className='font-bold text-gray-800 dark:text-white'>{project.tasks?.length || 0}</span>
                </div>
                ...
                {project.projectManager?.status === 'Pending' && (
                  <div className='flex items-center gap-3 text-sm'>
                    ...
                    <span className='text-yellow-600 dark:text-yellow-500 font-bold truncate underline underline-offset-4 decoration-yellow-600/40'>
                      {project.projectManager?.email?.split('@')[0] || 'Pending'}
                    </span>
                  </div>
                )}

                <div className='pt-3 border-t border-gray-50 dark:border-[#1a1a1a] space-y-2'>
                  <button onClick={() => handleProjectClick(project._id)} className='w-full flex items-center gap-3 px-3 py-2 rounded-xl text-gray-600 dark:text-gray-400 hover:bg-orange-50 dark:hover:bg-orange-500/10 hover:text-orange-600 transition-all text-xs font-bold'>
                    <FiSettings /> Environment Architect
                  </button>
                  <button onClick={() => handleTaskManagement(project._id)} className='w-full flex items-center gap-3 px-3 py-2 rounded-xl text-gray-600 dark:text-gray-400 hover:bg-orange-50 dark:hover:bg-orange-500/10 hover:text-orange-600 transition-all text-xs font-bold'>
                    <FiClipboard /> Workflow Orchestrator
                  </button>
                </div>
              </div>
              ...
              <div className="px-5 pb-5">
                {project.projectManager?.status === 'Pending' ? (
                  <button onClick={() => handleManagerAssignmentClick(project)} className='w-full cursor-pointer flex items-center justify-center gap-3 py-3 rounded-xl bg-orange-600 hover:bg-orange-700 text-white transition-all shadow-lg shadow-orange-600/20'>
                  ...
                  <div className='flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-[#151515] border border-gray-100 dark:border-[#1a1a1a]'>
                    ...
                    <div className='min-w-0'>
                      ...
                      <p className='text-xs font-bold text-gray-700 dark:text-gray-300 truncate'>{project.projectManager?.email || 'N/A'}</p>
                    </div>
                  </div>
                )}
              </div>
