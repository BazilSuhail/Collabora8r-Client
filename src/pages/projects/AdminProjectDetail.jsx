...
import { FaUserPlus, FaSearch, FaClipboardList, FaUsers } from 'react-icons/fa'
...
const AdminProjectDetail = () => {
  ...
  const handleSearch = async () => {
    ...
    setIsLoading(true)
    try {
      const { data } = await api('POST', '/admin-projects/get-searched-user', { email, projectId })
      setUser(data)
    } catch (err) {
  ...
  const handleAddUser = async () => {
    ...
    setSuccess('')
    try {
      const { data } = await api('POST', '/admin-projects/send-project-invitation', { userId: user.userId, projectId })
        ...
        <section className='lg:col-span-8 space-y-8'>
          ...
          <div className='bg-white dark:bg-[#0a0a0a] p-4 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 border border-gray-100 dark:border-[#1a1a1a] rounded-2xl md:rounded-3xl shadow-sm'>
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-semibold text-gray-800 dark:text-white tracking-tight uppercase">{project.name}</h1>
                <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 font-semibold mt-1">Project Details</p>
              </div>
            </div>
            <button onClick={() => setShowModal(true)} className='px-5 md:px-8 py-3 md:py-4 bg-orange-600 hover:bg-orange-700 text-white rounded-lg md:rounded-xl text-xs md:text-sm font-bold uppercase tracking-wider transition-all shadow-sm active:scale-95 whitespace-nowrap'>Edit</button>
          </div>
          ...
          <div className='bg-white dark:bg-[#0a0a0a] p-4 md:p-6 border border-gray-100 dark:border-[#1a1a1a] rounded-2xl md:rounded-3xl shadow-sm'>
            ...
            <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 leading-relaxed">{project.description}</p>
          </div>
          ...
          <div className='bg-white dark:bg-[#0a0a0a] p-4 md:p-6 border border-gray-100 dark:border-[#1a1a1a] rounded-2xl md:rounded-3xl shadow-sm min-h-[400px] flex flex-col'>
            <div className="flex items-center gap-3 md:gap-4 text-orange-600 mb-6 md:mb-8">
              <div className="w-10 h-10 md:w-11 md:h-11 bg-orange-100 dark:bg-orange-500/10 rounded-lg md:rounded-xl flex items-center justify-center"><MdMarkEmailUnread className="text-lg md:text-xl" /></div>
              <div>
            ...
            <div className="relative group mb-6 md:mb-8">
              <div className="absolute inset-y-0 left-0 pl-3 md:pl-4 flex items-center pointer-events-none"><FaSearch className="text-xs md:text-sm text-gray-400 group-focus-within:text-orange-500 transition-colors" /></div>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter email address" className="w-full pl-10 md:pl-12 pr-32 md:pr-40 py-3 md:py-4 bg-gray-50 dark:bg-[#151515] text-gray-800 dark:text-white text-sm md:text-base border border-gray-200 dark:border-[#1a1a1a] focus:border-orange-500/40 focus:ring-1 focus:ring-orange-500/20 rounded-lg md:rounded-xl outline-none transition-all font-medium" />
              <button onClick={handleSearch} className="absolute right-2 md:right-3 top-2 md:top-3 bottom-2 md:bottom-3 px-4 md:px-6 bg-orange-600 hover:bg-orange-700 text-white rounded-md md:rounded-lg text-xs md:text-sm font-bold uppercase tracking-wider transition-all shadow-sm active:scale-95">Search</button>
            </div>
              ...
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col md:flex-row md:justify-between md:items-center bg-gray-50 dark:bg-[#151515] p-4 md:p-6 rounded-lg md:rounded-xl border border-gray-100 dark:border-[#1a1a1a] gap-4 md:gap-6 group/user">
                <div className="flex items-center gap-3 md:gap-4">
                  <div className="relative">
                    <img src={`/avatars/${user.avatar}.jpg`} alt={user.name} className="w-14 h-14 md:w-16 md:h-16 rounded-full md:rounded-xl border-2 border-white dark:border-[#0a0a0a] shadow-md grayscale group-hover/user:grayscale-0 transition-all" />
                ...
                <button onClick={handleAddUser} className="w-full md:w-auto flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-700 text-white px-6 md:px-8 py-3 md:py-4 rounded-lg md:rounded-xl text-xs md:text-sm font-bold uppercase tracking-wider transition-all shadow-sm active:scale-95 whitespace-nowrap">
                  <FaUserPlus />
                  <span>Add</span>
                </button>
              </motion.div>
        ...
        <section className='lg:col-span-4 space-y-8'>
            ...
            {project.projectManager?.status === 'Pending' ? (
              <div className='flex flex-col gap-3'>
                <div className='flex items-center gap-2 px-3 py-2 bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded-lg text-xs font-medium border border-amber-200 dark:border-amber-500/20'>
                  <div className="w-1.5 h-1.5 bg-amber-500 rounded-full animate-ping" />Invitation Pending
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400 font-medium truncate">{project.projectManager?.email || 'N/A'}</p>
              </div>
            ) : project.projectManager ? (
              <div className="flex items-center gap-3 md:gap-4 group/mgr">
                <img src={`/avatars/2.jpg`} alt={project.projectManager.name} className="w-12 h-12 md:w-14 md:h-14 rounded-lg md:rounded-xl border-2 border-white dark:border-[#0a0a0a] shadow-sm grayscale group-hover/mgr:grayscale-0 transition-all duration-500" />
                <div className="min-w-0">
          ...
          <div className="p-4 md:p-6 bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-[#1a1a1a] rounded-2xl md:rounded-3xl shadow-sm">
            <div className="flex items-center gap-3 text-orange-600 mb-4 md:mb-6 border-b border-gray-100 dark:border-[#1a1a1a] pb-4 md:pb-6">
              ...
              <span className="ml-auto bg-gray-100 dark:bg-[#151515] text-gray-600 dark:text-gray-400 px-2.5 md:px-3 py-1 rounded-lg text-xs font-bold">{(project.team?.length || 0)}</span>
            </div>
              ...
              <div className="flex flex-col gap-3">
                {project.team.map((member, index) => (
                  <div key={index} className="flex items-center gap-3 md:gap-4 hover:bg-gray-50 dark:hover:bg-[#151515] transition-colors p-2 md:p-3 rounded-lg group/member">
                    <img src={`/avatars/${member.avatar || 1}.jpg`} alt={member.name} className="w-9 h-9 md:w-10 md:h-10 rounded-full border border-gray-200 dark:border-[#1a1a1a] shadow-sm grayscale group-hover/member:grayscale-0 transition-all flex-shrink-0" />
                    <div className="min-w-0">
          ...
          <div className="p-4 md:p-6 bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-[#1a1a1a] rounded-2xl md:rounded-3xl shadow-sm">
            ...
            {project.tasks?.length > 0 ? (
              <div className="flex flex-col gap-3">
                {project.tasks.slice(0, 8).map((task, index) => (
                  <div key={index} className="p-3 md:p-4 bg-gray-50 dark:bg-[#151515] hover:bg-white dark:hover:bg-[#0a0a0a] rounded-lg md:rounded-xl border border-gray-100 dark:border-[#1a1a1a] transition-colors group/task shadow-sm">
                    <p className="text-xs md:text-sm font-semibold text-gray-800 dark:text-white mb-2 line-clamp-2 group-hover/task:text-orange-600 transition-colors">{task.title}</p>
                    <div className='flex justify-between items-center gap-2'>
                      ...
                      <p className="text-[9px] md:text-xs font-medium text-gray-500 dark:text-gray-400">{task.dueDate ? new Date(task.dueDate).toLocaleDateString() : 'No date'}</p>
                    </div>
                  </div>
                ))}
              </div>
